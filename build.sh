...
KEYSTORE="${CREW_STORY_KEYSTORE:-$SCRIPT_DIR/test.keystore}"
OUT_APK="$SCRIPT_DIR/CrewStory.apk"
...
if [ ! -f "$KEYSTORE" ]; then
    ...
    keytool -genkey -v -keystore "$KEYSTORE" -alias crewstory -keyalg RSA -keysize 2048 -validity 10000 \
        -dname "CN=CrewStory, OU=OpenSource, O=CrewPocket, L=Taipei, ST=Taiwan, C=TW" \
...
javac -d bin/classes -cp "$ANDROID_JAR:$SCRIPT_DIR/app/libs/*" bin/gen/com/crewpocket/story/R.java app/src/main/java/com/crewpocket/story/*.java

echo "3. Converting to DEX..."
d8 --output bin/ bin/classes/com/crewpocket/story/*.class app/libs/*.jar
...
apksigner sign --ks "$KEYSTORE" \
  --ks-pass "pass:${CREW_STORY_KEYSTORE_PASS:-password}" \
  --key-pass "pass:${CREW_STORY_KEY_PASS:-password}" \
  --out "$OUT_APK" \
  bin/unsigned.apk

# Copy to external sdcard Download
if [ -d "/sdcard/Download" ]; then
    cp -f "$OUT_APK" "/sdcard/Download/CrewStory-v${VERSION_NAME}.apk"
    cp -f "$OUT_APK" "/sdcard/Download/CrewStory.apk"
    echo "📦 Copied to /sdcard/Download/CrewStory-v${VERSION_NAME}.apk"
fi

# Copy to storage/downloads if available
if [ -d "/data/data/com.termux/files/home/storage/downloads" ]; then
    cp -f "$OUT_APK" "/data/data/com.termux/files/home/storage/downloads/CrewStory-v${VERSION_NAME}.apk"
    cp -f "$OUT_APK" "/data/data/com.termux/files/home/storage/downloads/CrewStory.apk"
    echo "📦 Copied to /data/data/com.termux/files/home/storage/downloads/CrewStory-v${VERSION_NAME}.apk"
fi

echo "✅ SUCCESS: Built CrewStory v${VERSION_NAME} ($OUT_APK)"
